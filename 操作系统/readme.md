...
#### 协程的原理
当出现I/O阻塞的时候，由协程的调度器进行调度，通过将数据流立刻yield掉（主动让出），并且记录当前栈上的数据，阻塞完后立刻再通知线程恢复栈，并把阻塞的结果放到这个线程上去跑，这样看上去好像跟写同步代码没有任何差别，这整个流程可以称为`coroutine`，而跑在由`coroutine`负责调度的线程称为`Fiber`。比如Golang里的go关键字其实就是负责开启一个`Fiber`，让`func`逻辑跑在上面。
...
| 调度所属  | 由OS的内核完成 | 由用户完成 |
| 切换开销 | 设计模式切换（由用户态切换到内核态）、16个寄存器、PC、SP等寄存器的刷新等 | 只有三个寄存器的值修改（程序计数器PC、栈顶指针SP、数据寄存器DX） |
...
自旋CAS（也就是不成功就一直循环执行直到成功）如果长时间不成功，会给CPU带来非常大的执行开销。如果JVM能支持处理器提供的pause指令那么效率会有一定的提升，pause指令有两个作用，第一，它可以延迟流水线执行指令（de-pipeline），使CPU不会消耗过多的执行资源，延迟的时间取决于具体实现的版本，在一些处理器上延迟时间是零。第二，它可以避免在退出循环的时候因内存顺序冲突（memory order violation）而引起CPU流水线被清空（CPU pipeline flush），从而提高CPU的执行效率。
...
描述进程内内存使用情况的重要字段。用于详细区分进程内存的共享与私有、干净与脏页的状态。

- Shared_Clean：表示被多个进程共享的、且未被修改过的内存页大小

- Shared_Dirty：表示被多个进程共享的、且已被修改过的内存页大小

- Private_Clean：表示仅被当前进程独占的、且未被修改过的内存页大小

- Private_Dirty：表示仅被当前进程独占的、且已被修改过的内存页大小



## fork和vfork的区别

- fork的父子进程有独立的内存空间，vfork父子进程共享内存空间；

- fork父子进程可以并发执行，vfork先执行子进程，父进程阻塞，调用exec()系列函数或exit()终止后才运行父进程；

- vfork专门为了子进程创建后立刻调用exec（执行新程序）的场景设计，避免fork复制整个内存空间的开销，但是后面fork采用copy-on-write技术进行优化，fork基本可以取代vfork了。



## fork和clone的区别

- fork的父子进程有独立的内存空间，clone可以通过设置 CLONE_VM、CLONE_FILES、CLONE_SIGHAND 等标志位，控制子进程与父进程共享内存、文件描述符、信号处理程序等资源；

- fork实际是通过调用clone来实现的，只是传递了特定的标志位，pthread也是通过调用clone来创建线程的


## write/fwrite/sync/fsync/fflush的区别

- write：系统调用，直接写到内核缓冲区，没到磁盘，掉电可能丢数据；
- fwrite：C标准库函数，写到用户缓冲区，不触发系统那个调用，不进内核，内部满了会自动调用write；
- sync：系统调用，把整个系统所有脏缓存刷到磁盘，满、全局、不推荐业务使用；
- fsync：系统调用，只刷指定fd对应的文件，确保文件数据+元数据都落盘，数据库、日志必用；
- fflush：把用户缓冲刷到内核，内部本质就是调用write；

### 流程

fwrite -> 用户缓冲区 -> fflush -> 内核缓冲区 -> fsync -> 磁盘


## CPU affinity（亲和性）

把线程绑定在一个cpu上，减少切换，让cpu缓存一直热；